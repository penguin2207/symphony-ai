...
def main(wav_in, wav_out):
    sr, data = wav_to_arr(wav_in)
    print(data.shape)
    data_filt = gaussian_filt(sr, data)
    write(wav_out, sr,data_filt)
...
def to_midi(p, mid_out, t=[]):
    ...
    for n in notes:
        MyMIDI.addNote(track, channel, round(n), time, duration, volume)
        print(round(n))
        print(librosa.midi_to_note(round(n)))
        time = time + 1
...
if __name__ == '__main__':
    ...
    c_maj_notes = [130.8,	146.8,	164.8,	174.6,	196.0,	220.0,	246.9,	261.6]
    midi_score = to_midi(c_maj_notes, args.midi_out)
