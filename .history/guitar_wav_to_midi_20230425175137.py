...
if __name__ == '__main__':
    ...
    for i in range(data_filt_pad.shape[0] // (window+3)):
        ...
        if(abs(np.asanyarray(data_filt_pad[i*window:(i+1)*window]).mean())>5*10**(-14)):
           print(5*10**(-14))
           print(abs(np.asanyarray(data_filt_pad[i*window:(i+1)*window]).mean()))
           print("Note On")
        else: 
            print(abs(np.asanyarray(data_filt_pad[i*window:(i+1)*window]).mean()))
            print("Note Off")
        means+=window*(abs(np.asanyarray(data_filt_pad[i*window:(i+1)*window]).mean()))
