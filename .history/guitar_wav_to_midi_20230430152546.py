...
if __name__ == '__main__':
    ...
    if len(data_sf.shape) > 1 and data_sf.shape[1] > 1: #mono mixdown
        data_sf = data_sf.mean(axis=1)
    data_sf_filt = butter_lowpass_filter(data_sf, cutOff, rate, order)

    # peak normalize audio to -1 dB
    peak_normalized_audio = pyln.normalize.peak(data_sf_filt, -1.0)

    # measure the loudness first 
    meter = pyln.Meter(rate) # create BS.1770 meter
    loudness = meter.integrated_loudness(data_sf_filt)
    
    # loudness normalize audio to -12 dB LUFS
    loudness_normalized_audio = pyln.normalize.loudness(data_sf_filt, loudness, -12.0)

    write(args.wav_out, rate,loudness_normalized_audio)
