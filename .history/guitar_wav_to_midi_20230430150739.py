...
if __name__ == '__main__':
    ...
    peak_normalized_audio = pyln.normalize.peak(data_filt, -1.0)

    # measure the loudness first 
    meter = pyln.Meter(fs) # create BS.1770 meter
    loudness = meter.integrated_loudness(data_filt)
    
    # loudness normalize audio to -12 dB LUFS
    loudness_normalized_audio = pyln.normalize.loudness(data_filt, loudness, -12.0)
    ...
    for j in range(data_filt_pad.shape[0] // (window+3)):
        ...
        meter2 = pyln.Meter(rate, block_size=0.200)
        loudness = meter2.integrated_loudness(data_filt_pad[j*window:(j+1)*window])
        #if(loudness>5*10**(-14)):
           #print("Note On")
        #else: 
            #print("Note Off")
