...
if __name__ == '__main__':
    ...
    pad = np.zeros(100)
    data_filt_pad = np.append(pad, data_filt)
    print(signaltonoise_dB(data_filt_pad))

    plt.plot(data_filt_pad)
    plt.show()
    
    # proc_fft(args.wav_in, args.wav_out)
    
   
    window = int(50/1000*fs)
    pitches = []
    means = []
    for i in range(data_filt_pad.shape[0] // (window+3)):
        print("Window: ", (i+1), "/", (data_filt_pad.shape[0] // (window+3)))
        if(abs(np.asanyarray(data_filt_pad[i*window:(i+1)*window])**2)>5*10**(-14)):
           print(5*10**(-14))
           print(np.abs(np.asanyarray(data_filt_pad[i*window:(i+1)*window]))**2)
           print("Note On")
        else: 
            print(np.abs(data_filt_pad[i*window:(i+1)*window])**2)
            print("Note Off")
        means+=window*[(abs(np.asanyarray(data_filt_pad[i*window:(i+1)*window]))**2)]
        pitches.append(augmented_detect_pitch_CMNDF(data_filt_pad, window, i*window, fs))
        #closest(guitar_notes, pitch_detect(data, window, i*window, fs))
    print(means)
    plt.plot(data_filt_pad, label = "orig_signal")
