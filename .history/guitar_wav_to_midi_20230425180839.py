...
if __name__ == '__main__':
    ...
    for i in range(data_filt_pad.shape[0] // (window+3)):
        ...
        (f, S)= scipy.signal.welch(data_filt_pad[i*window:(i+1)*window], fs, nperseg=4*1024)
        if(np.abs(data_filt_pad[i*window:(i+1)*window])**2>5*10**(-14)):
           print("Note On")
           plt.semilogy()
        else: 
            print("Note Off")
        means+=window*[np.abs(data_filt_pad[i*window:(i+1)*window])**2]
