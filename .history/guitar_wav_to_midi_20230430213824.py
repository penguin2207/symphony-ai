...
if __name__ == '__main__':
    ...
    y_min =plt.axis()[2]
    y_max =plt.axis()[3]
    onsets = [x//float(fs) for x in get_onsets(args.wav_in)]
    for stamp in onsets:
        plt.plot([stamp, stamp], [y_min, y_max], '-r')
    plt.plot(t_pitches, on_offs*y_max, '-g', label = "note_on")
    plt.legend()
    plt.show()
    ...
    for i,pitch in enumerate(p):
        ...
        added=False
        for ons in onsets:
            if abs(t_pitches[i]-ons)<=0.01 and pitch!=0:
