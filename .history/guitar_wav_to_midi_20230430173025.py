...
if __name__ == '__main__':
    ...
    for j in range(data_filt_pad.shape[0] // (window+3)):
        ...
        pitches += (window)*augmented_detect_pitch_CMNDF(data_filt_pad, window, j*window, fs))
    ...
    t_pitches = [ float(t) * ((window)) / fs for t in range(len(data_filt_pad)) ]
    pad_size_pitches=int((len(t_pitches)-len(pitches)))
    pitches = np.pad(maxs, (0, pad_size_pitches), 'constant')
